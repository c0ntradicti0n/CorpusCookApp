...
class TrueFormatUpmarker:
    ...
    def convert_and_index (self, pdf_path):
        self.path = pdf_path
        html_path = self.transform(pdf_path)
        indexed_words = self.load(html_path)
    ...
    def load(self, path):
        ...
        #span_space_splitter = lambda divcontent: TrueFormatUpmarker.seq_split(divcontent,
        #                                              cond=lambda x: isinstance(x, Tag) and x.attrs['class'] == ['_'])
        #self.index_words(self.reading_sequence_text, splitter=span_space_splitter)
        ...
        with open(f"{path}.pdf2htmlEX.html", "w", encoding='utf8') as file:
    ...
    def save_doc_json(self):
        doc_dict = {"text": " ".join (list(self.indexed_words.values()))}
        with open(self.path + ".pdf2htmlEX.json", "w", encoding="utf8") as f:
            f.write(json.dumps(doc_dict))

if __name__ == '__main__':
    tfu = TrueFormatUpmarker()
    pprint(tfu.convert_and_index(pdf_path='/home/stefan/cow/pdfetc2txt/docs/0013.pdf'))
    tfu.save_doc_json()

    pprint(tfu.convert_and_index(pdf_path='/home/stefan/cow/pdfetc2txt/docs/what is string theory.pdf'))
